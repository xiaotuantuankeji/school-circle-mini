...
<script>
	...
	export default {
							...
							success: r => {
								...
								if (!this.hasSel) {
									...
									if (this.selWidth && this.selHeight) {
										const selWidth = this.selWidth.indexOf('rpx') >= 0 ? parseInt(this.selWidth) * this.pxRatio :
					...
					success: (r) => {
						...
						this.btop(r).then((r) => {
							if (this.exportWidth && this.exportHeight) {
								const ctxCanvas = this.ctxCanvas;
      ...
      fStart(e) {
      	...
      	if (touch1) {
            const x = touch1.clientX - touch0.clientX,
      ...
      // #ifdef MP-WEIXIN
        ...
        fMove(e) {
            ...
            const x=touch0.clientX - this.touch0.clientX,
			...
			async fEnd(e) {
				...
				if (self.scaleSize < 1) {
					const style = self.selStyle;
					const imgRadio = self.imgWidth / self.imgHeight;
					//高长宽短
					...
					const endWidth = this.posWidth - this.postFirst.posWidth,
						firstWidth = this.postFirst.left - this.postFirst.posWidth;
					const endHeight = this.posHeight - this.postFirst.posHeight,
